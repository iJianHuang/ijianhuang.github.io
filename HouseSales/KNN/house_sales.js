...
async function Run2() {
    ...
    tf.util.shuffle(pointsArray); // attn: in-place shuffle
    pointsSelectedArray = pointsArray.slice(0, 5000);



    const pointsTensor2d = tf.tensor2d(pointsSelectedArray);
    const [pointsFeatures, pointsLabels] = tf.split(pointsTensor2d, [3, 1], 1);
    //pointsTensor.slice([1,0], [3,4]).print();

    const {mean, variance} = tf.moments(pointsFeatures, 0); //  The dimension(s) along with to compute
    const standardizedPointsFeatures = pointsFeatures
        .sub(mean)
        .div(variance.pow(.5));

    const predictionPoint = [47.38, -122.44, 1970]; //, 705000];
    const predictionPointFeature1d = tf.tensor1d(predictionPoint);
    const standardizedPredictionPointFeature1d = predictionPointFeature1d.sub(mean).div(variance.pow(.5));

    const mse1d = standardizedPointsFeatures
    ...
    const price = topKValues.sum().dataSync()[0] / topKValues.shape[0];

    //let price = (sum / topKIndexes.length).toFixed(2).toString();
}

async function KNN() {
    const houseSalesDataset = tf.data.csv("https://ijianhuang.github.io/HouseSales/kc_house_data.csv");

    const selectedDatasetParameters = ["lat", "long", "sqft_living", "price"];
    const pointsDataset = houseSalesDataset.map(record => { 
        const row = [];
        selectedDatasetParameters.forEach(p => row.push(record[p]));
        return row;
    });
    const pointsArray = await pointsDataset.toArray();    
    tf.util.shuffle(pointsArray); // attn: in-place shuffle
    ...
    const pointsTensor2d = tf.tensor2d(pointsSelectedArray);
    const [pointsFeatures, pointsLabels] = tf.split(pointsTensor2d, [selectedDatasetParameters.length - 1, 1], 1);
    //pointsTensor.slice([1,0], [3,4]).print();
...
async function Run3() {
    ...
    const pointsDataset = houseSalesDataset.map(record => ({
        x: record.long,
        y: record.lat,
        class: record[className] >= 1000000? "+1M" : "<1M" // record[className] // 
    }));
    points = await pointsDataset.toArray();
    if (points.length % 2 !== 0) {
        points.pop();
    ...
    plotClasses(points, className, 800, false);
    

    let pointsArrayPlot = standardizedPointsFeatures.arraySync();
    let pointsPlot = pointsArrayPlot.map(p => ({ 
        x: p[0], 
        y: p[1]
        //class: p[className] >= 1000000? "+1M" : "<1M" 
    }));
    await plotClasses(pointsPlot, className, 800, false);

    plot(pointsPlot, 'Test');
}
